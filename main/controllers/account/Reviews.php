	...
	private function _addReview() {
		...
		if ($this->validateForm() === TRUE) {
			...
			$add['review_text'] 		= $this->input->post('review_text');
 			...
 			$location = $this->Locations_model->getLocation($add['location_id']);         
            $add['restaurant_id'] =  $location['restaurant_id'];

			if ($this->Reviews_model->saveReview(NULL, $add, $location['restaurant_id'])) {
