...
class Local_module extends Main_Controller {
	...
	public function index($module = array()) {
		...
		$total_reviews = $this->Reviews_model->getTotalLocationReviews($this->location->getId(),$this->location->getRestaurantId());
	...
	public function searchAllByDistance() {
		$this->load->library('user_agent');
		$json = array();			
	...
	//log_message('info',print_r($this->location->CI->session->local_info['geocode']['lat'],true));
	//log_message('info',print_r($this->location->getLatLng($this->input->post('search_query')),true));
		$search_info = $json['search_info'] = $this->location->getLatLng($this->input->post('search_query'));
		$lat = $search_info['lat'];
		$lng = $search_info['lng'];

		$redirect = '';
		if (!isset($json['error'])) {
			$redirect = $json['redirect'] = site_url('local/all?sort_by=distance&lat='.$lat.'&lng='.$lng);
		}
	...
	//log_message('info',print_r($json,true));
