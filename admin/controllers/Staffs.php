...
class Staffs extends Admin_Controller {
	...
	public function index() {
		...
		$results = $this->Locations_model->getLocations($this->user->restaurant_id);
		foreach ($results as $result) {
			$data['locations'][] = array(
	...
	public function edit() {
       ...
       // }
	   	...
	   	$data['staff_is_account_manager']  =      $this->Staff_restaurant_model->isStaffAccountManager($staff_info['staff_id']);
		...
		// Load restaurant list staff has acces
		$data['staff_restaurant_acces'] = array();
		if($staff_info){
		    $results = $this->Staff_restaurant_model->getRestaurantListByStaff($staff_id);
		   	foreach ($results as $result) {
				$data['staff_restaurant_acces'][] = array(
		...
		// All restaurant list
	    $restaurant_list = $this->user->getRestauratList();
		for($i =0 ; $i< count($restaurant_list); $i++ ){
			if(!$staff_info){
				if($restaurant_list[$i]['restaurant_id']== $this->user->restaurant_id){
				   $restaurant_list[$i]['current']  = true;
				}			
				$restaurant_list[$i] ['acces'] = true;
			}else{
				$data['staff_default_restaurant_id'] = $staff_info['restaurant_id'];
				// Este caso es para el editar vemos cual es el current restaurante.
				if( $restaurant_list[$i]['restaurant_id']==$data['staff_default_restaurant_id'] ){
					$restaurant_list[$i] ['current'] = true;
				}else{
                    $restaurant_list[$i] ['current'] = false;
				}
				// Marcar si tiene acceso.
				$restaurant_list[$i] ['acces'] = false;
				for($j= 0 ; $j < count($data['staff_restaurant_acces']);$j++){
					if($restaurant_list[$i]['restaurant_id'] == $data['staff_restaurant_acces'][$j]['restaurant_id']){
						$restaurant_list[$i] ['acces'] = true;
					}
				}	           
			}		
	...
	private function _saveStaff($staff_email, $username) {
        if ($this->validateForm($staff_email, $username) === TRUE) {
            ...
            if ($staff_id = $this->Staffs_model->saveStaff($this->input->get('id'), $this->input->post())) {
				...
				//Default Restaurant and List Acces
				$restaurant_for_acces = array();
				foreach($_POST as $key => $value){	
					if(substr_count($key,'rest_acces_')>=1){
						$restaurant_for_acces[]=$value;
					}
				}

				$results = $this->Locations_model->getLocationIdByIdRestaurant($_POST['rest_default']);
				if(count($results)>0){
				   	$this->Staffs_model->setLocation($results[0]['location_id'],$staff_id);
				}else{
					$this->Staffs_model->setLocation('0',$staff_id);
				}				
				
				$this->Staff_restaurant_model->setRestaurantListForStaff($restaurant_for_acces,$staff_id);
	...
	private function validateForm($staff_email = FALSE, $username = FALSE) {
		...
		$this->form_validation->set_rules('rest_default', 'rest_default', 'callback__check_default');	//validate form
	...
	public function _check_default($str) {
			...
			$rest_default = $_POST['rest_default'];

			if(count($restaurant_for_acces)==0){
				$this->form_validation->set_message('_check_default', $this->lang->line('alert_no_restaurant_selected'));
			    return FALSE;
		    }

			//var_dump(array_search($rest_default,$restaurant_for_acces));die;
			
			if(array_search($rest_default,$restaurant_for_acces)===false){
				$this->form_validation->set_message('_check_default', $this->lang->line('alert_no_restaurant_default'));
			    return FALSE;
			}
