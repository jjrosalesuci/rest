...
class Orders extends Admin_Controller {
	...
	public function index() {
		...
		$order_dates = $this->Orders_model->getOrderDates($this->user->restaurant_id);
	...
	public function edit() {
		$order_info = $this->Orders_model->getOrder((int) $this->input->get('id'),null,$this->user->restaurant_id);

		$is_from_monitor =  $this->input->get('from');
		...
		if ($this->input->post() AND $this->_updateOrder() === TRUE) {
			if ($this->input->post('save_close') === '1') {				
				if($this->input->post('from') === 'monitor'){
				   redirect('monitor');
				}else{
				   redirect('orders');
				}
	...
	public function invoice() {
		...
		$invoice_info = $this->Orders_model->getInvoice($this->uri->rsegment('4'),$this->user->restaurant_id);
		...
		$data['cart_items'] = array();
		$cart_items = $this->Orders_model->getOrderMenus($invoice_info['order_id'], $this->user->restaurant_id);
		$menu_options = $this->Orders_model->getOrderMenuOptions($invoice_info['order_id'],$this->user->restaurant_id );
		foreach ($cart_items as $cart_item) {
			$option_data = array();
		...
		$data['totals'] = array();
		$order_totals = $this->Orders_model->getOrderTotals($invoice_info['order_id'],$this->user->restaurant_id);
	...
	private function validateForm() {
		...
		$this->form_validation->set_rules('order_time', 'lang:label_order_time', 'required|callback__check_time');
	...
	public function _check_time($order_time) {		
		if(preg_match('/^(?:[01][0-9]|2[0-3]):[0-5][0-9]$/',$order_time)) {
			  	$current   = new DateTime("now");
				$order_t   = new DateTime($order_time);								
				if($current > $order_t){
					$this->form_validation->set_message('_check_time', $this->lang->line('error_time_invalid'));
					return FALSE;
				}
		}else{
				$this->form_validation->set_message('_check_time', $this->lang->line('error_format_invalid'));
				return FALSE;
		}
		return TRUE;
	}
