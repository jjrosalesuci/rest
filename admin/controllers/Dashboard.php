...
class Dashboard extends Admin_Controller {
	...
	public function changerestaurant (){
		$restaurant_id     = (int) $this->input->get('restaurant_id');
		$restaurant_name   = $this->input->get('restaurant_name');			
		if($this->Staff_restaurant_model->isStaffOnRestaurant($this->user->getStaffId(), $restaurant_id)){
		  $this->user->setCurrentRestaurant($restaurant_id,$restaurant_name );
		}		
		redirect('dashboard');
	}
	...
	public function index() {
		...
		$filter_for_staff['restaurant_id'] = $this->user->restaurant_id;
		$staffGroups = $this->Staffs_model->getList($filter_for_staff);
		$staff_id_arr = [];
		foreach($staffGroups as $index=>$value){
			$staff_id_arr[] = $value['staff_id'];
		}
		$filter['staff_list']= $staff_id_arr;
		...
		// Setup Steps
		$data['setup_steps']=  $this->Setup_restaurant_complete_model->getStepsForRestaurant($this->user->restaurant_id);
	...
	public function statistics() {
		...
		$result = $this->Dashboard_model->getStatistics($stat_range, $restaurant_id);
	...
	public function chart() {
        ...
        if (count($dateRanges) <= 1) {
            for ($i = 0; $i < 24; $i++) {
                $data = $this->Dashboard_model->getTodayChart($i, $restaurant_id);
                $data['time'] = mdate('%H:%i', mktime($i, 0, 0, date('n', $timestamp), date('j', $timestamp), date('Y', $timestamp)));
                $results[] = $data;
            }
        } else {
            for ($i = 0; $i < count($dateRanges); $i++) {
                $data = $this->Dashboard_model->getDateChart($dateRanges[$i], $restaurant_id);
