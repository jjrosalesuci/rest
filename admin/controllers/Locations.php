...
class Locations extends Admin_Controller {
	...
	public function index() {
		...
		$total_rows = $this->Locations_model->getCount($filter);
		if($total_rows==0){
			redirect('locations/edit');
		}
		else{		
			$results = $this->Locations_model->getList($filter);
			$id_location = $results[0]['location_id'];
			redirect("locations/edit?id=$id_location");
		}
        ...
        $data['tables'] = array();
        $tables = $this->Tables_model->getTables($this->user->restaurant_id);
        if ($tables) {
	...
	public function edit() {
		...
		$location_info = $this->Locations_model->getLocation((int) $this->input->get('id'),$this->user->restaurant_id);
	...
	private function _saveLocation() {
    	if ($this->validateForm() === TRUE) {
			...
			if ($location_id = $this->Locations_model->saveLocation($this->input->get('id'), $this->input->post(), $this->user->restaurant_id)) {
