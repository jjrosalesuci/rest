...
class Reviews_model extends TI_Model {
	...
	public function getCount($filter = array()) {
	    ...
	    if ( ! empty($filter['restaurant_id'])) {
		  	$this->db->where($this->db->dbprefix('reviews').'.restaurant_id', $filter['restaurant_id']);
		}
	...
	public function getReviews($customer_id = FALSE, $restaurant_id = FALSE) {
	...
	public function getTotalsbyId($location_id = FALSE, $restaurant_id=FALSE) {
	...
	public function getReview($review_id, $customer_id = FALSE, $sale_type = FALSE, $restaurant_id) {
		if ( ! empty($review_id)) {
			$this->db->from('reviews');
			$this->db->join('locations', 'locations.location_id = reviews.location_id', 'left');
	...
	public function getReviewDates($restaurant_id) {
	...
	public function getTotalLocationReviews($location_id, $restaurant_id = FALSE) {
		$this->db->where('location_id', $location_id);
		$this->db->where('review_status', '1');
		$this->db->from('reviews');
	...
	public function checkReviewed($sale_type = 'order', $sale_id = '', $customer_id = '', $restaurant_id = FALSE) {
	...
	public function saveReview($review_id, $save = array(), $restaurant_id) {
		...
		if (APPDIR === ADMINDIR AND isset($save['review_status']) AND $save['review_status'] === '1') {
			...
			$this->db->set('review_status', '1');							// por ahora lo guardo siempre aprobado
		}
