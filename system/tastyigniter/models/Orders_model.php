...
class Orders_model extends TI_Model {
    ...
    public function getCount($filter = array()) {
        ...
        if ( ! empty($filter['restaurant_id'])) {
		    $this->db->where( $this->db->dbprefix('orders').'.restaurant_id', $filter['restaurant_id']);
		}
    ...
    public function getList($filter = array()) {
        ...
        if ($this->db->limit($filter['limit'], $filter['page'])) {
            ...
            if ( ! empty($filter['only_actives'])) {
                  $this->db->where('orders.status_id !=', '15');
                  $this->db->where('orders.status_id !=', '19');
            }
    ...
    public function getOrder($order_id = FALSE, $customer_id = '', $restaurant_id =false) {
        if ( ! empty($order_id)) {
            $this->db->from('orders');
            $this->db->join('statuses', 'statuses.status_id = orders.status_id', 'left');
            $this->db->join('locations', 'locations.location_id = orders.location_id', 'left');
            ...
            if ($restaurant_id != false) {
		        $this->db->where($this->db->dbprefix('orders').'.restaurant_id', $restaurant_id);
		    }
    ...
    public function getInvoice($order_id = NULL,$restaurant_id) {
    ...
    // Revisar quien llama a este
    public function getCheckoutOrder($order_id, $customer_id,$restaurant_id) {
    ...
    public function getOrderMenus($order_id, $restaurant_id = FALSE) {
    ...
    public function getOrderMenuOptions($order_id, $restaurant_id = FALSE) {
        $result = array();
    ...
    public function getOrderTotals($order_id, $restaurant_id = FALSE) {
    ...
    // Revisar quien llama es este
    public function getOrderCoupon($order_id, $restaurant_id) {
    ...
    public function getOrderDates($restaurant_id) {
    ...
    // Revisar quien llama a este
    public function isOrderPlaced($order_id,$restaurant_id) {
        $this->db->from('orders');
        $this->db->where('order_id', $order_id);
    ...
    public function updateOrder($order_id = NULL, $update = array()) {
        ...
        // Check this
        if (isset($update['order_time'])) {
            $this->db->set('order_time', $update['order_time']);
        }
    ...
    public function addOrder($order_info = array(), $cart_contents = array()) {
        ...
        if (isset($order_info['restaurant_id'])) {
           $this->db->set('restaurant_id', $order_info['restaurant_id']);
        }
