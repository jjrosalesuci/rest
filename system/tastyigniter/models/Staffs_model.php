...
class Staffs_model extends TI_Model {
	...
	public function getCount($filter = array()) {
		...
		if (!empty($filter['restaurant_id'])) {
				$table = $this->db->dbprefix('staffs');
			    $stafft_id_column_onstaff = "`$table`.`staff_id`"; 
				$restaurant_id = $filter['restaurant_id'];
				$table = $this->db->dbprefix('staff_restaurant');
				$this->db->where("$stafft_id_column_onstaff IN (SELECT `staff_id` FROM `$table` WHERE `restaurant_id`=$restaurant_id)", NULL, FALSE);
         }
	...
	public function findByEmail($user_email){
		$this->db->where('staff_email',$user_email);
		$this->db->from('staffs');
		$query = $this->db->get();
		$result = array();
		if ($query->num_rows() > 0) {
	...
	public function saveStaff($staff_id, $save = array()) {
		...
		if (isset($save['rest_default'])) {
			$this->db->set('restaurant_id', $save['rest_default']);
		}
	...
	public function setLocation($location_id,$staff_id){		
		    $this->db->set('staff_location_id', $location_id);
		    $this->db->where('staff_id', $staff_id);
			$query = $this->db->update('staffs');
	}
