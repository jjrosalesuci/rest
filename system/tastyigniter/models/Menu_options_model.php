...
class Menu_options_model extends TI_Model {
	...
	public function getCount($filter = array()) {
		...
		$this->db->where('restaurant_id', $filter['restaurant_id']);
	...
	public function getOption($option_id, $restaurant_id) {
		$this->db->from('options');
		$this->db->where('option_id', $option_id);
	...
	public function getMenuOptions($menu_id = FALSE, $restaurant_id = FALSE) {
		$results = array();
	...
	public function getAutoComplete($filter_data = array()) {
		if (is_array($filter_data) AND ! empty($filter_data)) {
			...
			if ( ! empty($filter_data['restaurant_id'])) {
		    	$this->db->where('restaurant_id', $filter_data['restaurant_id']);
			}
	...
	public function saveOption($option_id, $save = array(), $restaurant_id) {
