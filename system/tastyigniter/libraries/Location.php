...
class Location {
	...
	public function __construct() {
		...
		//TODO Revisar aca tambien
		!empty($this->locations) OR $this->locations = $this->getLocations(null);
    }
	...
	public function initialize($local_info = array()) {
		...
		if (!$is_loaded) {
			if (!isset($local_info['location_id']) OR empty($this->locations[$local_info['location_id']])) {
				...
				// TODO Verificar que vamos hacer con el restaurante
				
				$this->restaurant_id = $location['restaurant_id'];
...
//log_message('info', print_r($output,true));
	...
	public function checkDistance($lat = FALSE,$long =FALSE) {
		$distance = 0;
	...
	private function getLocations() {
		if (empty($this->locations)) {
			$this->CI->load->model('Locations_model');
			//TODO revisar aca tambien
			$locations = $this->CI->Locations_model->getLocations(null);
